...
class BannerController extends ChangeNotifier {
  ...
  Future<void> getBannerList(BuildContext context, bool reload) async {
      if (bannerServiceInterface == null) {
        print('ERROR: BannerServiceInterface is null');
        return;
      }
      ApiResponse apiResponse = await bannerServiceInterface!.getList(context);
  ...
  Future<void> getSellerWiseBannerList(int sellerId) async {
    ApiResponse apiResponse = await bannerServiceInterface!.getSellerWiseBannerList(sellerId);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _mainBannerList = [];
      _footerBannerList = [];
