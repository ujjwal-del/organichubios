      ...
      builder: (BuildContext context) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                getTranslated('Oops, you missed something', context) ?? 'Oops, you missed something',
                style: titilliumSemiBold.copyWith(color: Theme.of(context).primaryColor, fontSize: Dimensions.fontSizeLarge),
              ),
              const SizedBox(height: 4),
              Text(
                getTranslated('Please complete the following', context) ?? 'Please complete the following before continuing:',
              ...
              Wrap(
                ...
                children: missing.map((item) => Chip(
                  label: Text(item, style: titilliumRegular.copyWith(color: Theme.of(context).primaryColor)),
                    ...
                    return Container(height: cartList.isNotEmpty? 160 : 0, padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge,
                      ...
                      Column( children: [
                        ...
                        InkWell(onTap: () {
                              ...
                              // Check if configModel is available
                              if (configProvider.configModel == null) {
                                showCustomSnackBar('Configuration not loaded yet', context);
                                return;
                              }
                               ...
                               if(configProvider.configModel!.shippingMethod =='sellerwise_shipping'){
                                for(int index = 0; index < cartProductList.length; index++) {
                                  for(CartModel cart in cartProductList[index]) {
                                    if(cart.productType == 'physical' && sellerGroupList[index].shippingType == 'order_wise'  &&
                                        Provider.of<ShippingController>(context, listen: false).shippingList != null &&
                                        Provider.of<ShippingController>(context, listen: false).shippingList!.isNotEmpty &&
                                        index < Provider.of<ShippingController>(context, listen: false).shippingList!.length &&
                                ...
                                for(int index = 0; index < sellerGroupList.length; index++) {
                                  ...
                                  if (sellerGroupList[index].minimumOrderAmountInfo != null) {
                              ...
                              for(int index = 0; index < sellerGroupList.length; index++) {
                                if(sellerGroupList[index].quantity != null && 
                                   sellerGroupList[index].productInfo != null && 
                                   sellerGroupList[index].productInfo!.totalCurrentStock != null &&
                                   sellerGroupList[index].quantity! > sellerGroupList[index].productInfo!.totalCurrentStock! && 
                                   sellerGroupList[index].productType =="physical") {
                              ...
                              for(int index = 0; index < sellerGroupList.length; index++) {
                                if (sellerGroupList[index].shop?.vacationEndDate != null && 
                                    sellerGroupList[index].shop?.vacationStartDate != null &&
                                    sellerGroupList[index].shop?.vacationStatus != null &&
                                    sellerGroupList[index].shop?.temporaryClose != null) {
                              ...
                              if(configProvider.configModel?.guestCheckOut == 0 && !Provider.of<AuthController>(context, listen: false).isLoggedIn()){
                                ...
                                changeColor();
                                _showReminder([getTranslated('select_all_shipping_method', context) ?? 'Shipping method']);
                              }
                                ...
                                _showReminder([getTranslated('Please select shipping method', context) ?? 'Shipping method']);
                              }else if(minimum){
                                changeColor();
