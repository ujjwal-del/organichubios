...
class CategoryRepository implements CategoryRepoInterface {
  ...
  Future<ApiResponse> getList(BuildContext context) async {
    try {
      ...
      final url = '${AppConstants.categoriesUri}$guestId';
      print('DEBUG: Category API call - URL: $url, guestId: $guestId, isLoggedIn: ${Provider.of<AuthController>(context, listen: false).isLoggedIn()}');
      final response = await dioClient!.get(url);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      print('DEBUG: Category API error: $e');
